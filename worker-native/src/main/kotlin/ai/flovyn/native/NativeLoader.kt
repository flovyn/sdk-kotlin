 ...
 * - macOS x86_64: libflovyn_worker_ffi.dylib
 * - macOS aarch64: libflovyn_worker_ffi.dylib
 * - Windows x86_64: flovyn_worker_ffi.dll
 * - Windows aarch64: flovyn_worker_ffi.dll
 */
object NativeLoader {
    ...
    fun ensureLoaded() {
        ...
        val (platform, libName) = when {
            os.contains("mac") && isArm64 -> "macos-aarch64" to "libflovyn_worker_ffi.dylib"
            os.contains("mac") && isAmd64 -> "macos-x86_64" to "libflovyn_worker_ffi.dylib"
            os.contains("mac") -> "macos-aarch64" to "libflovyn_worker_ffi.dylib" // Default to arm64 on macOS
            os.contains("linux") && isAmd64 -> "linux-x86_64" to "libflovyn_worker_ffi.so"
            os.contains("linux") && isArm64 -> "linux-aarch64" to "libflovyn_worker_ffi.so"
            os.contains("windows") && isArm64 -> "windows-aarch64" to "flovyn_worker_ffi.dll"
            os.contains("windows") && isAmd64 -> "windows-x86_64" to "flovyn_worker_ffi.dll"
            os.contains("windows") -> "windows-x86_64" to "flovyn_worker_ffi.dll" // Default to x86_64 on Windows
            else -> error("Unsupported platform: $os $arch")
        }

        // Check if library override is set (allows using a custom library location)
        val libraryOverride = System.getProperty("uniffi.component.flovyn_worker_ffi.libraryOverride")
